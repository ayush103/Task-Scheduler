        ...
        if ($result->num_rows > 0) {
          ...
          <thead>
            ...
            while($row = $result->fetch_assoc()) {
              ...
              $dldate1 = $row['C_date']; 
              $difference=strtotime($current_date) - strtotime($dldate1);

              if ($row['deadline'] > $cd) {
                echo"<tr><th scope='row'><button type='button' class='btn btn-default view_data' data-toggle='modal' data-target='#modal1' id=".$row['taskid']."><img src='image/edit.png'></button></th><td>".$row["tname"]."</td><td>".$row["deadline"]."<br>Created before ".$difference." Days</td></tr>";
              }
              else{
                echo"<tr style='background: #ff6961;'><th scope='row'><button type='button' class='btn btn-default view_data' data-toggle='modal' data-target='#modal1' id=".$row['taskid']."><img src='image/edit.png'></button></th><td style='color: #ffffff'>".$row["tname"]."</td><td style='color: #ffffff'>".$row["deadline"]."<br>Created before ".$difference." Days</td></tr>";
              }
            }
            echo "</tbody></table>";
        } 
